...
namespace IMLibrary.Net
{
    ...
    public partial class SockTCPServer : Component
    {
        ...
        /// 接受新的Socket通信连接请求
        /// </summary>
        public delegate void AcceptSocketEventHandler(object sender, IMLibrary.Net.SockEventArgs e);
        ...
        /// 连接客户端数量达到所设置的上限值
        /// </summary>
        public delegate void MaxConnectCountArrivalEventHandler(object sender, IMLibrary.Net.SockEventArgs e);
        ...
        private bool InitSocket(System.Net.IPAddress LocalIPAddress, int LocalPort)
        {
            ...
            {
                Active = false;
                Error(this, new IMLibrary.Net.SockEventArgs(1, ex.Message));
            }
            return Active;
        }
        ...
        private bool StartListen(int LimitAcceptCount)
        {
            ...
            {
                Active = false;
                Error(this, new IMLibrary.Net.SockEventArgs(2, ex.Message));
            }
            return Active;
        }
        ...
        private void CloseClientSocket(MyTcp myTcp)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    Close(myTcp, new IMLibrary.Net.SockEventArgs());//触发关闭事件
                }
                catch
                { }
            ...
            {
                Error(myTcp, new IMLibrary.Net.SockEventArgs(3, ex.Message));
            }
        }
        ...
        private void AsyncClientSocketReceive(IAsyncResult aResult)
        {
                ...
                if (myTcp.Socket != null)
                {
                    if (myTcp.Socket.Connected)
                    {
                        ...
                        if (iReceiveCount > 0)
                        {
                            ...
                            DataArrival(myTcp, new IMLibrary.Net.SockEventArgs(data,myTcp.IP,myTcp.Port));
            ...
            {
                //接收数据异常，则关闭该客户端连接
                //IMLibrary.Calculate.WirteLog("接收数据异常，则关闭该客户端连接"+ex.Source +ex.Message );
                CloseClientSocket(myTcp);
                Error(myTcp, new IMLibrary.Net.SockEventArgs(4, ex.Message));
            }
        }
        ...
        private void AsyncAcceptSocket(IAsyncResult aResult)
        {
                ...
                if (IsStopServer == false)//如果没有终止侦听
                {
                    ...
                    if (handler != null)
                    {
                        ...
                        if (handler.Connected)
                        {
                            ...
                            {
                                MaxConnectCountArrival(this, new IMLibrary.Net.SockEventArgs());//触发连接数量达到上限事件
                            ...
                            else
                            {
                                ...
                                AcceptClientSocket(myTcp, new IMLibrary.Net.SockEventArgs());//触发接收新连接事件
            ...
            catch (Exception ex)
            {
                //接受连接过程发生异常关闭连接
                Stop();
                Error(this, new IMLibrary.Net.SockEventArgs(5, ex.Message));
            }
        }
        ...
        private void AsyncClientSend(IAsyncResult aResult)
        {
            ...
            {
                Error(myTcp, new IMLibrary.Net.SockEventArgs(6, ex.Message));
            }
        }
        ...
        private void Send(byte[] Data, MyTcp myTcp)
        {
            ...
            {
                Error(myTcp, new IMLibrary.Net.SockEventArgs(7, ex.Message));
            }
        }
        ...
        public bool Stop()
        {
            ...
            if (_Socket != null)
            {
                ...
                {
                    Active = false;
                    Error(this, new IMLibrary.Net.SockEventArgs(8, ex.Message));
                }
        ...
        public void BroadcastMessage(byte[] Data)
        {
            for (int i = 0; i < _clientSocketList.Count; i++)
            {
                ...
                {
                    Error(myTcp, new IMLibrary.Net.SockEventArgs(9, ex.Message));
                }
            }
        }
        #endregion
