...
namespace IMLibrary.ExtRichTextBox
{
...
//						MessageBox.Show(reoObject.fileName );
        ...
        /// gis图片集合
        /// </summary>
        public IMLibrary.gifCollections gifs = new gifCollections();
        ...
        /// <returns></returns>
        public  IMLibrary.MyPicture findPic(uint Index)
        {
        ...
        public IMLibrary.MyPicture findPic(string MD5Value)
        {
            foreach (IMLibrary.MyPicture pic in gifs)
        ...
        public IMLibrary.MyPicture addGifControl(string imageMD5, Image image)
        {
        ...
        {
            string imageInfo = "";
            IMLibrary.ExtRichTextBox.REOBJECT reObject = new IMLibrary.ExtRichTextBox.REOBJECT();
            for (int i = 0; i < this.GetRichEditOleInterface().GetObjectCount(); i++)//循环访问richbox中所有图片或对像
            {
                this.GetRichEditOleInterface().GetObject(i, reObject, IMLibrary.ExtRichTextBox.GETOBJECTOPTIONS.REO_GETOBJ_ALL_INTERFACES);
