...
//using CSS.IM.XMPP.protocol.component;
...
namespace CSS.IM.XMPP
{
	...
	public class IqGrabber : PacketGrabber
	{
        ...
        public IqGrabber(XmppComponentConnection conn)
        {
            m_connection = conn;
#if MONOSSL
            conn.OnIq += new CSS.IM.XMPP.protocol.component.IqHandler(OnIqComponent);
#else
			conn.OnIq += new CSS.IM.XMPP.protocol.component.IqHandler(OnIq);
#endif
        }        
		...
		public void SendIq(IQ iq, IqCB cb, object cbArg)
		{
            ...
            if (cb != null)
            {
                ...
                //iq在CSS.IM.XMPP中发送Iq节的时候先iq.RemoveAttribute("to")
