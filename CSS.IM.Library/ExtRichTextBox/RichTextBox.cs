...
namespace CSS.IM.Library.ExtRichTextBox
{
	...
	{ 
        ...
        {
            foreach (CSS.IM.Library.MyPicture pic in gifs)
            {
                if (pic.ImageMD5.Length > 3)
                {
                    try
                    {
                        pic.Image.Dispose();
...
//						MessageBox.Show(reoObject.fileName );
        ...
        /// gis图片集合
        /// </summary>
        public CSS.IM.Library.gifCollections gifs = new gifCollections();
        ...
        /// 在Gifs中查找图片控件
        /// </summary>
        /// <param name="ID">GIF ID</param>
        /// <returns></returns>
        public  CSS.IM.Library.MyPicture findPic(uint Index)
        {
            foreach (CSS.IM.Library.MyPicture pic in gifs)
                if (pic.Index == Index)
                    return pic;
        ...
        public CSS.IM.Library.MyPicture findPic(string MD5Value)
        {
            foreach (CSS.IM.Library.MyPicture pic in gifs)
                if (pic.ImageMD5 == MD5Value)
                    return pic;
            return null;
        }
        #endregion
        ...
        /// </summary>
        /// <returns></returns>
        public CSS.IM.Library.gifCollections GetExistGifs()
        {

            //Debug.WriteLine("对象总数：" + this.GetRichEditOleInterface().GetObjectCount());

            CSS.IM.Library.gifCollections tempGifs = new gifCollections();
            
            CSS.IM.Library.ExtRichTextBox.REOBJECT reObject = new  REOBJECT ();
            
            CSS.IM.Library.MyPicture pic;
            for (int i = 0; i < this.GetRichEditOleInterface().GetObjectCount(); i++)//循环访问richbox中所有图片或对像
            {

                this.GetRichEditOleInterface().GetObject(i, reObject, CSS.IM.Library.ExtRichTextBox.GETOBJECTOPTIONS.REO_GETOBJ_ALL_INTERFACES);
                pic = this.findPic(reObject.dwUser);//在图片集合中查找此图片
                if (pic != null && pic.ImageMD5.Length>3)//如果此图片名在集合中
                {
        ...
        public string  GetImageInfo()
        {
            ...
            try
            {
                CSS.IM.Library.ExtRichTextBox.REOBJECT reObject = new CSS.IM.Library.ExtRichTextBox.REOBJECT();
                for (int i = 0; i < this.GetRichEditOleInterface().GetObjectCount(); i++)//循环访问richbox中所有图片或对像
                {
                    this.GetRichEditOleInterface().GetObject(i, reObject, CSS.IM.Library.ExtRichTextBox.GETOBJECTOPTIONS.REO_GETOBJ_NO_INTERFACES);
        ...
        public string GetUserImageInfo()
        {
            string imageInfo = "";
            CSS.IM.Library.ExtRichTextBox.REOBJECT reObject = new CSS.IM.Library.ExtRichTextBox.REOBJECT();
            for (int i = 0; i < this.GetRichEditOleInterface().GetObjectCount(); i++)//循环访问richbox中所有图片或对像
