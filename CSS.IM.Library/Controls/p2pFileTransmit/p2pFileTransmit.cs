...
namespace CSS.IM.Library.Controls
{
    ...
    public partial class p2pFileTransmit : Component
    {
        ...
        /// 一次读写文件的大小，定义为10M
        /// </summary>
        private int _maxReadWriteFileBlock = 5242880*4;//5M
        [Category("全局设置")]
        ...
        private UserInfo _selfUserInfo;//自己的在线信息
        private UserInfo _OppositeUserInfo;//对方的在线信息
        ...
        private NetCommunicationClass netClass = NetCommunicationClass.None;//文件传输所采用的通信协议,初始为通讯没有成功
        ...
        public void SetParameter(bool IsSend, string FullFileName, string FileName, int FileLen, string fileExtension, string FileMD5Value,UserInfo selfUserInfo, UserInfo OppositeUserInfo)
        {
            ...
            this.FileLenStr = Calculate.GetSizeStr(FileLen);//获得文件尺寸字符串
        ...
        private void sendFile(int currTransmittedLen)
        {
            ...
            this.sendData(new msgFile((byte)ProtocolFileTransmit.GetFileBlock, this.serverSelfID, this.serverOppositeID, (long)currTransmittedLen, buffer));//发送已读取的文件数据给对方
        ...
        private void sendRequestGetFileData()
        {
            ...
            this.OutTime = 0;
            this.sendData(new msgFile((byte)ProtocolFileTransmit.FileTransmit, this.serverSelfID, this.serverOppositeID, (long)this.currGetPos, new byte[0])); //请求对方发送文件数据包
        ...
        /// 处理对方发送文件数据块
        /// </summary>
        private void ReceivedFileBlock(msgFile msg)//当对方发送文件数据块过来
        {
            if (msg.pSendPos > this.currGetPos)//如果发送过来的数据大于当前获得的数据
            {
                ...
                {
                    msg.InfoClass = (byte)ProtocolFileTransmit.FileTranstmitOver;
        ...
        private void onFileTransmitted()
        {
            ...
            try
            {
                if (this.netClass == NetCommunicationClass.TCP)
        ...
        private void DataArrival(msgFile msg, NatClass netClass, System.Net.IPAddress Ip, int Port)
        {
            ...
            {
                case (byte)ProtocolFileTransmit.GetFileTransmitProxyID://获得自己从服务器上获得中转服务ID 
                    {
                        this.serverSelfID = msg.SendID;
                        if (this.serverOppositeID != -1)//如果是发送方获得中转服务ID，则告诉对方开始收接文件
                        {
                            this.netClass = NetCommunicationClass.TCP;//标识当前通信协议为TCP
                    ...
                    break;
                case (byte)ProtocolFileTransmit.FileTransmit://获得文件传输请求
                    {
                        //Calculate.WirteLog("发送文件给对方");
                        this.sendFile((int)msg.pSendPos);//发送文件给对方
                    }
                    break;
                case (byte)ProtocolFileTransmit.GetFileBlock :// 获得对方传输的文件数据包
                    {
                        //Calculate.WirteLog("收到文件数据");
                        this.ReceivedFileBlock(msg);//对方发送文件数据过来,保存数据到文件
                    }
                    break;
                case (byte)ProtocolFileTransmit.BeginTransmit://连接已经建立，对方要求开始接收文件
                    {
                        this.serverOppositeID = msg.SendID;//获得对方ID
                        if (netClass == NatClass.Tcp)//如果是TCP通信
                        {
                            this.netClass = NetCommunicationClass.TCP;//采用TCP协议传输文件 
                        }
                        else
                        {
                            this.netClass = NetCommunicationClass.WanNoProxyUDP;//采用UDP协议传输  
                        }
                    ...
                    break;
                case (byte)ProtocolFileTransmit.FileTranstmitOver ://文件传输结束
                    {
                ...
                case (byte)ProtocolFileTransmit.HandshakeLAN://收到对方局域网UDP握手数据
                    {
                        this._OppositeUserInfo.LocalIP = Ip;//重新设置对方的局域网IP
                        this.OppositeUDPPort = Port;//重新设置对方的局域网UDP端口
                        msg.InfoClass =(byte)ProtocolFileTransmit.IsOppositeRecSelfLanUDPData;//告诉对方收到握手数据信息
                    ...
                    break;
                case (byte)ProtocolFileTransmit.IsOppositeRecSelfLanUDPData://对方收到自己发送的局域网UDP握手数据
                    {
                        this.netClass = NetCommunicationClass.LanUDP;//标识与对方建立局域网通信成功
                ...
                case (byte)ProtocolFileTransmit.HandshakeWAN ://收到对方广域网UDP握手数据
                    {
                        this._OppositeUserInfo.IP = Ip;//重新设置对方的广域网IP
                        this.OppositeUDPPort = Port;//重新设置对方的广域网UDP端口
                        msg.InfoClass = (byte)ProtocolFileTransmit.IsOppositeRecSelfWanUDPData ;//告诉对方收到握手数据信息
                    ...
                    break;
                case (byte)ProtocolFileTransmit.IsOppositeRecSelfWanUDPData://对方收到自己发送的局域网UDP握手数据
                    {
                        this.netClass = NetCommunicationClass.WanNoProxyUDP;//标识与对方建立广域网直接通信成功
                        if (this._IsSend)//如果是发送文件的一方，且对方告之收到自己的握手数据，则通信通道打通
                        {
                ...
                case (byte)ProtocolFileTransmit.GetUDPWANInfo://获得服务器返回的文件传输套接字广域网UDP端口
                    {
        ...
        public void setFileTransmitGetUdpLocalPort(int Port, bool udpHandshakeInfoClass)
        {
            ...
            else if (udpHandshakeInfoClass)//采用广域网方式通信
            {
                ...
                msg.InfoClass = (byte)ProtocolFileTransmit.GetUDPWANInfo;//通信协议
        ...
        private void UdpHandshake()
        {
            try
            {
                ...
                {
                    msg.InfoClass = (byte)ProtocolFileTransmit.HandshakeLAN;
                    this.sockUDP1.Send(this._OppositeUserInfo.LocalIP, this.OppositeUDPPort, msg.getBytes());
                }
                else if (UdpHandshakeInfoClass)//如果为广域网握手
                {
                    msg.InfoClass = (byte)ProtocolFileTransmit.HandshakeWAN;
                    this.sockUDP1.Send(this._OppositeUserInfo.IP, this.OppositeUDPPort, msg.getBytes());
                }
            }
            catch { }
        ...
        {
            if (this.netClass != NetCommunicationClass.None)//如果UDP通信成功
            {
